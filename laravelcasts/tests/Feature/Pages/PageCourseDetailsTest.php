...
it('includes paddle checkout button', function () {
    // dd(config('services.paddle.vendor-id', 'vendor-id'));
    $vendor_id = config('services.paddle.vendor-id', 'vendor-id');
    $course = Course::factory()
        ->released()
        ->create([
    ...
    get(route('page.course-details', $course))
        ...
        ->assertSee("Paddle.Setup({ vendor: $vendor_id});", false)
        ->assertSee('<a href="#!" data-product="product-id" data-theme="none"', false);
});
