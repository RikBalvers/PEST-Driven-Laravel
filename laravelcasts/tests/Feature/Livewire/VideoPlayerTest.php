...
it('shows given video', function () {
    ...
    // Act & Assert
    Livewire::test(VideoPlayer::class, ['video' => $video])
        ->assertSeeHtml('<iframe class="w-full mb-4 rounded aspect-video md:mb-8" src="https://player.vimeo.com/video/' . $video->vimeo_id);
});
...
it('does not include route for current video', function () {
    ...
    Livewire::test(VideoPlayer::class, ['video' => $course->videos()->first()])
        ->assertDontSeeHtml([
            ...
            route('page.course-videos', [$course, Video::where('title', 'First video')->first()]),
            route('page.course-videos', [$course, Video::where('title', 'Second video')->first()]),
            route('page.course-videos', [$course, Video::where('title', 'Third video')->first()]),
        ]);
});
